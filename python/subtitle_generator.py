...
def generate_word_json(
    ...
    """
    Generate word-level JSON for Remotion subtitle overlay.
...
def generate_subtitles(
    ...
    """
    Generate both SRT and word JSON from alignment result.

    Args:
        alignment: AlignmentResult from Whisper
        srt_output_path: Path for SRT file
        words_output_path: Path for word JSON file

    Returns:
        Tuple of (srt_path, words_json_path)
    """
    # Generate SRT from segments
    srt_path = generate_srt(alignment.segments, srt_output_path)

    # Generate word-level JSON
    words_path = generate_word_json(alignment.words, words_output_path)
