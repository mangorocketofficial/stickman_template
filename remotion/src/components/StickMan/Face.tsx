...
// Auto-detect face contrast color from body color
function getContrastColor(bodyColor: string): string {
  const hex = bodyColor.replace('#', '');
  const r = parseInt(hex.slice(0, 2), 16);
  const g = parseInt(hex.slice(2, 4), 16);
  const b = parseInt(hex.slice(4, 6), 16);
  // Luminance check: dark body → white face features, light body → dark face features
  const luminance = (r * 299 + g * 587 + b * 114) / 1000;
  return luminance < 128 ? '#FFFFFF' : '#1a1a2e';
}
...
export const Face: React.FC<FaceProps> = ({
  ...
  const faceColor = backgroundColor || getContrastColor(color);
